...
Пробегись по коду и убедись что нет других хардкоженных секретов. Быстрый чек в PowerShell:

```powershell
# Ищем оставшиеся токены/IP/логины в коде
Get-ChildItem -Recurse -Include *.cs,*.json,*.axaml,*.md | 
    Select-String -Pattern "ptlc_|213\.152\.43|00b1d7f9|mgr\.hosting-minecraft"
```

Если что-то нашлось — вынеси в `launcher.secrets.json` по той же схеме, что и остальное. В идеале **ничего** не должно найтись.
